        ...
        if (!this.input.value.startsWith(":")) {
            ...
            if (Object.prototype.hasOwnProperty.call(this.responseCache, this.input.value)) {
                ...
                if (this.input.value.length >= 3) {
                    ...
                    const response = await fetch(`https://nominatim.openstreetmap.org/search?q=${this.input.value}&format=json`).then(res => res.json());
    ...
    showInfo(info, activeItem) {
        ...
        if (Array.isArray(info)) {
            this.info = _react2.default.createElement(
                ...
                info.map(e => _react2.default.createElement(
                    'li',
                    {
                        key: e.place_id,
